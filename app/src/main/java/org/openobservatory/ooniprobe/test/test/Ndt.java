...
public class Ndt extends AbstractTest {
	...
	public Ndt(Context c, PreferenceManager pm, Gson gson) {
		super(c, pm, gson, NAME, MK_NAME, R.string.Test_NDT_Fullname, 0);
		if (!pm.isNdtServerAuto()) {
			this.settings.options.server = pm.getndtServer();
			this.settings.options.port = pm.getndtServerPort();
		}
	}
	...
	@Override public void run(Result result, int index, TestCallback testCallback) {
		...
		run(new org.openobservatory.measurement_kit.nettests.NdtTest(), result, index, testCallback);
	}
