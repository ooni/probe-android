...
final class PESession implements OONISession {
    ...
    @Override
    public OONIMakeSubmitterTask newMakeSubmitterTask(long timeout) throws OONIException {
        try {
            return new PEMakeSubmitterTask(session.newMakeSubmitterTask(timeout));
        } catch (Exception exc) {
